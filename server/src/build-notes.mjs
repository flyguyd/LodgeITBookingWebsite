...
export const BUILD_NOTES = [
  ...
  },
  {
    key: '0.1.89',
    version: '0.1.89',
    date: '2026-09-06T17:55:00+02:00',
    changes: [
      { headline: 'THE SUITES SELECTOR IS A CURRENCY SELECTOR NOW, ON BOTH PAGES (Dave, 2026-09-06: "change the suites selector for a currency selector (keep the style). Select from the exchange rates we have pushed from Lodge Ops. If a currency other than ZAR is selected then recalculate all amounts using the current exchange rate ... The calculated amounts are for display purposes only"). The desktop bar\'s Suites dropdown is a Currency dropdown in the same glass; the mobile page\'s Suites stepper is a Currency stepper in the same box, \u2212/+ walking the list. Rand first, then every currency Lodge Ops lists. Choosing one redraws every amount on the page \u2014 the cards, the calendar cells, the summary, the totals \u2014 at the pushed rate, and nothing underneath moves: the search, the hold and the booking are rand, and only the CODE the guest chose travels with them. The choice is remembered in the browser and rides the address. Retrieve booking shows a hold at the rate of ITS OWN DAY and sets the selector to the currency it was taken in.', detail: 'Pairs with engine 0.1.88 (GET /api/booking/currencies) and Lodge Ops 1.3.69 (migration 396). server/src/lib.mjs: FORWARD_ROUTES gains GET /api/public/currencies \u2192 /api/booking/currencies. core.js: a display-currency state { code, rate, at, locked } behind money() and moneyC() \u2014 toDisplay(amount, currency) multiplies a rand amount by the chosen rate on its way to the screen and passes an amount already in another currency untouched; symbolFor() (R, \u20ac, \u00a3, $ \u2026, else the code); loadCurrencies() fetches the list once and fills a code chosen before the rates arrived; setDisplayCurrency(code, rate?, at?) \u2014 with a rate it is LOCKED (Retrieve booking) until the guest picks again; a bk:currency DOM event announces every change. calendar.js fmtShort() converts the day cells the same way. review.js gains repaint(): the picks and the totals redrawn in the new currency, the agreement box and the hold and checkout sections left standing because the rand figures under them did not change. booking.js (desktop): #fCurrency dressed by the same glassSelect as Nights; fRooms stays in the DOM HIDDEN as the value the search, the URL (suites=) and the hold snapshot still read \u2014 several suites are the Advanced search\'s business; snapshotState().form.currency carries the code; restoreHold() applies hold.displayCurrency / displayRate / displayRateAt (falling back to the snapshot\'s code) and sets the selector; the URL carries cur= when not rand; localStorage bk_currency remembers it; the last renderResults() payload is kept and redrawn on a change. m/booking.js: the same, with a .stepper.currency whose \u2212/+ cycle the codes, excluded from the numeric stepper wiring. VERIFIED on the Lodge Ops e2e rig: 70.*.2b\u20132e on desktop AND mobile in Chromium (the selector where Suites was, rand first then EUR and GBP, no Suites control shown, EUR turning the Lagoon card into \u20ac720 = 13,800 \u00d7 the pushed rate and back to R13,800, the choice remembered), 30.8b/30.12f (a EUR hold stamped by Lodge Ops and carried on retrieve), 40.24b (a GBP checkout); case 70 is 88 checks.' },
    ],
  },
  {
    key: '0.1.90',
    version: '0.1.90',
    date: '2026-09-06T19:35:00+02:00',
    changes: [
  ...
  },
  {
    ...
    changes: [
      { headline: 'C7 AND E2 CLOSED \u2014 THE KEYBOARD STAYS INSIDE AN OPEN DIALOG, A SKIP LINK, THE RESULTS ARE ANNOUNCED; AND CAMPAIGN DEEP LINKS (Dave, 2026-09-07: "Fix C7. Fix E2."). Both builds. Every modal (the hold, what-is-a-hold, terms, success and retrieve dialogs) and both lightboxes (the suite\u2019s story and Compare these rates) now trap Tab / Shift+Tab inside themselves while open, carry aria-modal, and hand focus back to where it was when they close. A "Skip to the search" link is the first thing the keyboard meets, off-screen until focused. A visually hidden live region reads the answer to a search aloud ("3 suites for 12 Jan \u2014 15 Jan \u00b7 3 nights" / "No suites are available for those dates"). Deep links for an advert or a channel: ?suite=<roomTypeId> opens that suite\u2019s story once the results are up (scrolled to its card), ?plan=<planId> lands every card on that plan, ?code= fills the code box; a link carrying only those still searches, on the default dates. The one-shot suite and plan drop out of the shareable URL after the search; the code stays in it. utm_* / campaign were already recorded on the session. The levy now comes from the engine\u2019s quote when it carries one (engine 0.1.89) rather than from the replicated lodge settings.', detail: 'core.js trapFocus(el) (exported; the release restores focus), the levy from opt.levy / room.levyFromEngine; review.js trapModals() \u2014 a MutationObserver on each .hold-modal\u2019s hidden attribute, so every open/close path is covered; lightbox.js both open() functions trap on the backdrop and release in close(); index.html (both) a.skip-link and #resultsLive (role status, aria-live polite); booking.css (both) .skip-link / .sr-only; booking.js (both) deepLink from restoreFromUrl (suite, plan, code), planFor built from ?plan= for hydrateRooms, landDeepLink() after renderResults (card.dataset.suite on every card), announce(), updateUrl keeps code and drops suite / plan. Verified in Chromium from Lodge Ops: e2e 70.*.30\u201370.*.34 on both pages.' },
    ],
  },
];
