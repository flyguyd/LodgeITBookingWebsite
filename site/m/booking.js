...
(function () {
  ...
  /* THE PARTY as the page knows it: the steppers' single party, or — in the
  ...
  function partyNow() {
    ...
    return { adults: els.adults.textContent, children: els.children.textContent, infants: els.infants ? els.infants.textContent : '0' };
  }
  var media = {};
  ...
  /* One search answer turned into rooms the cards can render, priced for
     ONE party (plans, the cheapest applied, the refusal reason, the
     over-capacity flag, the party on each room). Shared by the single
     search, Retrieve booking and every room of the advanced search. */
  ...
  function renderRoom(room, nights, index) {
    ...
    /* ONE ribbon, ever (Dave, 2026-09-04): no availability → "Sorry no
       availability" alone; a refusal the engine explained → its words;
       otherwise factual scarcity only. */
    var soldOut = (!(room.available > 0) && room.availabilityKnown !== false) || !!room.restricted;
    var availUnknown = room.availabilityKnown === false && !room.restricted;
    var ribbon = null;
    if (soldOut) {
    ...
    /* WHAT THE RATE RULES SAID (Dave, 2026-08-31) — stay messages, an
       ...
       the itemised tap statement, which the inclusive display never
       attached — a rule's message must never depend on a tap. */
    ...
    if (soldOut) {
      ...
      if (room.overCapacity && window.BKAdv) {
        /* Too many guests for this suite (engine 028): a suite per room is
           the way forward, not a calendar (Dave, 2026-09-04). */
        avail.textContent = 'Try advanced search';
  ...
  if (window.BKAdv && els.advPanel) {
    window.BKAdv.attach({
      ...
      onToggle: function (on) {
        if (els.adv) els.adv.classList.toggle('on', on);
        if (on) { hideStates(); hideSummary(); if (window.BKReview) window.BKReview.close(); }
      },
      scrollTop: function () {
        try { form.scrollIntoView({ behavior: 'smooth', block: 'start' }); } catch (e) { /* fine */ }
      },
